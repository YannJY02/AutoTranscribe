...
6. Run `python3.11 scripts/agent_harness.py verify --issue "{{ issue.identifier }}" --mode full`. Run issue-specific acceptance checks too. For `exclusive-macos`, wrap installed-app, GUI, capture, or performance commands with `agent_harness.py lock --resource installed-app -- ...`; visible behavior requires after evidence and `proof.json`.
   If a bounded investigation cannot resolve a full-verification failure, run `python3.11 scripts/agent_harness.py blocked-handoff --issue "{{ issue.identifier }}" --manifest <failed-full-manifest>`, then report the evidence and stop changing files. The command binds the report to `.symphony/attempt.json`; do not synthesize that identity or a passed manifest. The trusted launcher checks blocked reports after its startup delay and on each healthy polling cycle (defaults: 180 seconds, then 60-second intervals, plus bounded controller and tracker latency). Symphony ends further turns when it next observes the withdrawn dispatch label; an active turn is not interrupted immediately. The `after_run` hook handles the same report as a fallback.
