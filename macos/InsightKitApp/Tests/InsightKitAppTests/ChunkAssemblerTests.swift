...
final class ChunkAssemblerTests: XCTestCase {
    func testEightSecondChunkBoundaryAndFlush() throws {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("ChunkAssemblerTests_\(UUID().uuidString)", isDirectory: true)

        let assembler = ChunkAssembler(chunkDurationSec: 8, sampleRate: 16_000, chunkDir: tmp)
        let samplesA = Array(repeating: Float(0.1), count: 200_000)
        let chunksA = try assembler.append(samples: samplesA)

        XCTAssertEqual(chunksA.count, 1)
        XCTAssertEqual(chunksA[0].startMs, 0)
        XCTAssertEqual(chunksA[0].endMs, 8_000)

        let samplesB = Array(repeating: Float(0.2), count: 60_000)
        let chunksB = try assembler.append(samples: samplesB)

        XCTAssertEqual(chunksB.count, 1)
        XCTAssertEqual(chunksB[0].startMs, 8_000)
        XCTAssertEqual(chunksB[0].endMs, 16_000)

        let tail = try assembler.flush(minDurationSec: 0.1)
        XCTAssertEqual(tail.count, 1)
        XCTAssertEqual(tail[0].startMs, 16_000)
        XCTAssertGreaterThan(tail[0].endMs, 16_000)
