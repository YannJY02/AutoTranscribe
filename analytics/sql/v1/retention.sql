...
cohort_observation AS (SELECT c.*,
 ...
 CASE WHEN julianday(:window_end)-julianday(c.cohort_day)>=29 AND julianday(date(:window_start))-julianday(c.cohort_day)<=28 THEN 1 ELSE 0 END observable_28d_flag
 FROM cohorts c)
SELECT SUM(c.observable_7d_flag) mature_7d_cohort_installations,
 ...
 SUM(CASE WHEN c.observable_28d_flag=1 AND EXISTS(SELECT 1 FROM observed_successes s WHERE s.installation_id=c.installation_id AND julianday(s.day)-julianday(c.cohort_day)=28) THEN 1 ELSE 0 END) retained_28d,
